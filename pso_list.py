...
class Particle:
    ...
    def __init__(self, no_dim, x_range, v_range, fitness):
        ...
        """
        self.x = [np.random.uniform(x_range[0], x_range[1], 1)[0] for i in range(no_dim)]
        self.v = [np.random.uniform(v_range[0], v_range[1], 1)[0] for i in range(no_dim)]
...
class Swarm:
    ...
    def __init__(self, no_particle, no_dim, x_range, v_range, iw_range, c, fitness):
        ...
        """
        self.p = [Particle(no_dim, x_range, v_range, fitness) for i in range(no_particle)]
        ...
        for i in range(max_iter):
            ...
            for particle in self.p:
                for d in range(self.no_dim):
                    ...
                    particle.v[d] = iw * particle.v[d] + (self.c0 * np.random.uniform(0.0, 1.0, 1)[0] * \
                    (particle.pbestpos[d] - particle.x[d])) + \
                    (self.c1 * np.random.uniform(0.0, 1.0, 1)[0] *  (self.gbestpos[d] - particle.x[d]))
